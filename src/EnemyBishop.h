#ifndef _ENEMY_BISHOP_H
#define _ENEMY_BISHOP_H
#include "Enemy.h"
#include "Entity.h"
#include "Point.h"
#include "Wall.h"
...
// Enemy
// Represents a bishop enemy, moves diagonally
class EnemyBishop: public Enemy {
public:
    EnemyBishop();
    ~EnemyBishop();
...
private:
	...
	bool check(Wall **walls, int wallCount, Enemy **enemies, int enemyCount, Player *player, int width, int height, int moveX, int moveY, std::string direction);
	int moveX;
	int moveY;
};
